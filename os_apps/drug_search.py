...
def drug_search(genes_list, df):
    ...
    if len(no_gene) == 1:
        no_result = f'{no_gene[0]} is not a known drug target.'
    elif len(no_gene) > 1:
        no_result = f'{", ".join(no_gene)} are not known drug targets.'
    else:
        no_result = None
...
def app():
    ...
    st.title('Gene Drug Target Search')
    st.write('Provide a gene or list of genes to search our druggable gene database.')
