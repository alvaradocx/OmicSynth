...
def get_top_genes(df, diseases, omics_list=None, hla=False, create_chart=False, heidi=0.01, pval=0.05):
    ...
    ndd_omics = ['Cerebellum_metaBrain', 'Spinalcord_metaBrain', 'brain_eMeta', 'Cortex_metaBrain',
                 ...
                 'methylation_brain_mMeta', 'methylation_blood_mcrae', 'blood_eQTLgen', 'chromatin_blood_Bryois',
                 'Nerve_Tibial_GTEx']
    ...
    if omics_list == 'all':
        for disease in diseases:
            for omic in all_omics:
                temp_df = smr_df.query(f"Disease == '{disease}' & Omic == '{omic}' & p_SMR_multi < {pval} &"
                                       f" p_HEIDI > {heidi}")
                top_gene = temp_df[temp_df['p_SMR_multi'] == temp_df['p_SMR_multi'].min()]
                top_gene = top_gene[['Omic', 'Disease', 'annotated_gene', 'topSNP', 'b_GWAS', 'p_GWAS', 'b_SMR',
                                     'p_SMR_multi', 'p_HEIDI']]
    ...
    elif omics_list is not None:  # Custom omics list
        for disease in diseases:
        ...
        for disease in diseases:
            for omic in ndd_omics:
                temp_df = smr_df.query(f"Disease == '{disease}' & Omic == '{omic}' & p_SMR_multi < {pval} "
                                       f"& p_HEIDI > {heidi}")
                top_gene = temp_df[temp_df['p_SMR_multi'] == temp_df['p_SMR_multi'].min()]
                top_gene = top_gene[['Omic', 'Disease', 'annotated_gene', 'topSNP', 'b_GWAS', 'p_GWAS',
                                     'b_SMR', 'p_SMR_multi', 'p_HEIDI']]
    ...
    if omics_list == 'all':
        for disease in diseases:
            for omic in all_omics:
                ...
                top_snp = top_snp[['Omic', 'Disease', 'annotated_gene', 'topSNP', 'b_GWAS', 'p_GWAS',
                                   'b_SMR', 'p_SMR_multi', 'p_HEIDI']]
        ...
        for disease in diseases:
            for omic in omics_list:
                temp_df = smr_df.query(f"Disease == '{disease}' & Omic == '{omic}' & p_SMR_multi < {pval}"
                                       f" & p_HEIDI > {heidi}")
                top_snp = temp_df[temp_df['p_SMR_multi'] == temp_df['p_SMR_multi'].min()]
                top_snp = top_snp[['Omic', 'Disease', 'annotated_gene', 'topSNP', 'b_GWAS', 'p_GWAS', 'b_SMR',
                                   'p_SMR_multi', 'p_HEIDI']]
...
def app():
    ...
    path = './druggable_genome.csv'
    drug_df = pd.read_csv(path, sep=',')
    st.session_state['drugdf'] = drug_df

    st.title('Top Genes and Top SNPs')
    st.write('This interactive tool will allow you to extract either the top genes or SNPs across available diseases '
             'or available omics.')

    with st.sidebar:
        with st.form('query selection'):
            gene_or_not = st.radio("Extract top genes or top SNPs?", ('Genes', 'SNPs'))
            all_or_some = st.radio("Select which subset of omics to use", ('All available omics',
                                                                           'NDD-related omics', 'Custom'),
                                   help='NDD-related omics consist of omics related to brain areas,blood,and nerves.')
        ...
        if st.session_state['query_submit'] == 'run':
            with st.form('parameter_selection'):
                ...
                unique_dx.append('NDDs')
                unique_dx.append('Immuno')
                unique_dx.append('Intermediate')
                ...
                elif "NDDs" in diseases:
                    diseases = ['AD', 'ALS', 'FTD', 'LBD', 'PD', 'PSP']
                    st.session_state['dx_list'] = diseases

                elif "Immuno" in diseases:
                    diseases = ['CED', 'CRDIBD', 'RA', 'SLE', 'PS']
                    st.session_state['dx_list'] = diseases

                elif "Intermediate" in diseases:
                    diseases = ['CVD', 'HLP', 'HTN', 'MS', 'T2D']
                    st.session_state['dx_list'] = diseases
                ...
                if st.session_state['top_submit'] == 'run':
                    if gene_or_not == 'Genes' and all_or_some == 'All available omics':
                        ...
                        top_genes_df, top_gene_chart = get_top_genes(main_df, st.session_state['dx_list'],
                                                                     omics_list='all', hla=True,
                                                                     create_chart=True,
                                                                     heidi=st.session_state['heidi'],
                                                                     pval=st.session_state['pval'])
                        ...
                        top_genes_df, top_gene_chart = get_top_genes(main_df, st.session_state['dx_list'], hla=True,
                                                                     create_chart=True,
                                                                     heidi=st.session_state['heidi'],
                                                                     pval=st.session_state['pval'])
                        ...
                        top_snps_df, top_snps_chart = get_top_snps(main_df, st.session_state['dx_list'],
                                                                   omics_list='all', hla=True, create_chart=True,
                                                                   heidi=st.session_state['heidi'],
                                                                   pval=st.session_state['pval'])
    ...
    with st.container():
        if st.session_state['top_submit'] == 'run':
            ...
            # download 
            output_name = st.text_input('Please provide an output file name if you would like to download your results'
                                        ' dataframe', placeholder='example.csv')
            st.session_state['df_name'] = output_name

            if st.session_state['df_name']:
                st.download_button(label="Download data as CSV", data=convert_df(st.session_state['top_df']),
                                   file_name=st.session_state['df_name'], mime='text/csv')

            st.header('Result Chart')
            st.dataframe(st.session_state['top_chart'])

            chart_name = st.text_input('Please provide an output file name if you would like to download'
                                       ' your results chart', placeholder='example.csv')
            st.session_state['chart_name'] = chart_name

            if st.session_state['chart_name']:
                st.download_button(label="Download data as CSV", data=convert_df(st.session_state['top_chart']),
                                   file_name=st.session_state['chart_name'], mime='text/csv')
    ...
    with st.container():
        ...
        with col1:
            ...
            if st.session_state['drug_status'] == 'run':
                ...
                drug_filename = st.text_input('Please provide an output file name for the druggable gene results',
                                              placeholder='example.csv')
                st.session_state['drug_name'] = drug_filename

                if st.session_state['drug_name']:
                    st.download_button(label="Download data as CSV", data=convert_df(st.session_state['gene_drugdf']),
                                       file_name=st.session_state['drug_name'], mime='text/csv')

        with col2:
            with st.form('value counts'):
                st.subheader("Gene/SNP counts")
                st.write('This tool will return the counts of how many times a gene or SNP shows up in your top '
                         'results.')
